...
function loadCart() {
    ...
    if (Object.keys(cart).length == 0) {
        ...
        out += '<h4>Корзина пуста <a href="./products.html" class="link">закупиться</a></h4>';
        out += '</div>';

        document.querySelector('.total_panel').style.display = 'none';
        ...
        for (key in cart) {
            ...
            if (data[key].group[0] == 'notebook') {
                ...
                type = 'телефон';
            } else if (data[key].group[0] == 'headphones') {
                type = 'наушники';
            }
