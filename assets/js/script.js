...
function getLinkData(link, name) {
    if (link.target.classList.contains(name)) {
        let group = link.target.getAttribute('data-group');
        let producer = link.target.getAttribute('data-producer');

        if (group != null && producer != null) {
            sessionStorage.setItem('product_group', group);
            sessionStorage.setItem('product_producer', producer);
        }
    }
}
...
function showModalBar(id) {
    ...
    if (id == undefined) {
        ...
        for (key in data) {
            if (key == id) {
                out += '<img src="./content/images/products/' + data[key].images[0] + '" alt="' + data[key].name + '">';
...
const footer_profile_btn = document.querySelector('.footer_profile_btn');
const menu_profile_btn = document.querySelector('.menu_profile_btn');

footer_profile_btn.addEventListener('click', goToProfile);
menu_profile_btn.addEventListener('click', goToProfile);
