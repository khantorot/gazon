...
function checkProduct() {
    if (sessionStorage.getItem('product') != null) {
        product_id = sessionStorage.getItem('product');
    } else {
        product_id = '1-1';
    }
}
...
function showProduct() {
    ...
    data[product_id].images.forEach((element, index) => {
        out += '<div data-id="' + index + '" class="gallery_item"><img src="./content/images/products/' + element + '"></div>'
    });

    out += '</div>';
    out += '<h5 class="product_title">' + data[product_id].producer + '</h5>';
    out += '<div class="product_img">';
    out += '<img src="./content/images/products/' + data[product_id].images[0] + '">';
    out += '</div>';
    ...
    document.title = data[product_id].name;
}
...
gallery.addEventListener('click', function (e) {
    if (e.target.classList.contains('gallery_item')) {
        ...
        product_image.innerHTML = '<div data-id="' + id + '" class="gallery_item"><img src="./content/images/products/' + data[product_id].images[id] + '"></div>'
    }
});
...
function showTegs() {
    ...
    out += '<a href="./products.html"><span class="teg">все товары</span></a>';
